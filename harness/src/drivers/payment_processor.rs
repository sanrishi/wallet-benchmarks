...
impl PaymentProcessorDriver {
    ...
    fn seed_words_with_birthday(&self, birthday: u64) -> anyhow::Result<String> {
        let mnemonic = SeedWords::from_str(&self.seed_words)
            .context("failed to parse stored seed words for payment_processor")?;
        let mut seed = CipherSeed::from_mnemonic(&mnemonic, None)
            .context("failed to reconstruct cipher seed for payment_processor")?;
        let birthday = u16::try_from(birthday)
            .with_context(|| format!("birthday {birthday} exceeds u16 range for payment_processor"))?;
    ...
    fn key_manager(&self) -> anyhow::Result<KeyManager> {
        ...
        let wallet = WalletType::SeedWords(
            SeedWordsWallet::construct_new(cipher_seed)
                .map_err(|_| anyhow!("failed to construct seed-words wallet for payment_processor"))?,
        );
        KeyManager::new(wallet).context("failed to build key manager for payment_processor")
    }

    fn self_address_string(&self) -> anyhow::Result<String> {
        let mnemonic = SeedWords::from_str(&self.seed_words)
            .context("failed to parse stored seed words for payment_processor address")?;
        let cipher_seed = CipherSeed::from_mnemonic(&mnemonic, None)
            .context("failed to reconstruct cipher seed for payment_processor address")?;
        let wallet = WalletType::SeedWords(
            SeedWordsWallet::construct_new(cipher_seed)
                .map_err(|_| anyhow!("failed to construct seed-words wallet for payment_processor address"))?,
        );
        ...
        .context("failed to construct payment_processor self address")?;
    ...
    fn get_unspent_output_count(&self) -> anyhow::Result<u64> {
        let connection = Connection::open(self.database_path())
            .context("failed to open payment_processor database for output counting")?;
        let count: i64 = connection
            ...
            .context("failed to query unspent outputs from payment_processor database")?;
    ...
    async fn send_recipients(
        ...
        .context("failed to offline-sign locked batch transaction")?;
        ...
        let transaction_value = serde_json::to_value(&signed.signed_transaction.transaction)
            .context("failed to serialize signed batch transaction for HTTP submit")?;
